...
class IMG_Stegno:
    ...
    def main(self, root):
        ...
        encode = Button(frame,text="Encode", fg='#C4DEDE', command= lambda :self.encode_frame1(frame), padx=14,bg = '#353B3B' )
        encode.config(font=('Helvetica',14), bg='#424ADA')
        encode.grid(row=2, pady=10)
        decode = Button(frame, text="Decode", fg='#C4DEDE', command=lambda :self.decode_frame1(frame), padx=14,bg = '#353B3B')
        decode.config(font=('Helvetica',14), bg='#DB763C')
    ...
    def encode_frame1(self,F):
        ...
        button_bws = Button(F2,text='Select', fg='#C4DEDE', padx=14,command=lambda : self.encode_frame2(F2))
        button_bws.config(font=('Helvetica',18), bg='#2DA547')
        button_bws.grid()
        button_back = Button(F2, text='Cancel', fg='#C4DEDE', padx=14,command=lambda : IMG_Stegno.back(self,F2))
        button_back.config(font=('Helvetica',18),bg='#C93232')
    ...
    def decode_frame1(self,F):
        ...
        button_bws = Button(d_f2, text='Select', fg='#C4DEDE', padx=14, command=lambda :self.decode_frame2(d_f2))
        button_bws.config(font=('Helvetica',18), bg='#2DA547')
        button_bws.grid()
        button_back = Button(d_f2, text='Cancel', fg='#C4DEDE', padx=14, command=lambda : IMG_Stegno.back(self,d_f2))
        button_back.config(font=('Helvetica',18), bg='#C93232')
        button_back.grid(pady=15)
        button_back.grid()
        d_f2.grid()
    ...
    def encode_frame2(self,e_F2):
        ...
        if not myfile:
            ...
            text_a.grid()
            encode_button = Button(e_pg, text='Cancel', padx=14, command=lambda : IMG_Stegno.back(self,e_pg))
            encode_button.config(font=('Helvetica',14), bg='#C93232')
            data = text_a.get("1.0", "end-1c")
            button_back = Button(e_pg, text='Encode', padx=14, command=lambda : [self.enc_fun(text_a,my_img),IMG_Stegno.back(self,e_pg)])
            button_back.config(font=('Helvetica',14), bg='#2DA547')
            button_back.grid(pady=15)
            encode_button.grid()
            e_pg.grid(row=1)
            e_F2.destroy()
    ...
    def decode_frame2(self,d_F2):
        ...
        if not myfiles:
            ...
            button_back.config(font=('Helvetica',14),bg='#353B3B')
            button_back.grid(pady=15)
            button_back.grid()
            d_F3.grid(row=1)
            d_F2.destroy()
